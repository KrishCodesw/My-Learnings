# /*
# EC2 Instance 
# New Security Group - Open port 22,3000,443 https, 80 http
...
#   #   key_name="" either create a keypair from the aws ui  or generate aws_keypair resource in terraform
#   //give permissions if downloaded keypair chmod 400 file_name
...
resource "aws_instance" "tf_server" {
  ...
  user_data                   = <<-EOF
                                ...
                                # edit env vars
                                echo "DB_HOST=${rds_endpoint}" | sudo tee .env 
                                ...
                                npm install
                                EOF
  depends_on                  = [aws_s3_bucket.tf_s3_bucket] # attach s3 bucket meta arguement
  // creation of the ec2 instance now depends on the s3 bucket 
